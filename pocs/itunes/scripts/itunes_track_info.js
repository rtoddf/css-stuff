...
var ItunesTracks = new (function () {
  this.get = function (track_name) {
    ...
    var searchString =
      baseSearchString + '?term=' + searchTerm + '&entity=musicTrack';

    //http://itunes.apple.com/search?term=We+Don't+Talk+About+Bruno&entity=musicTrack
      ...
      .then(function (data) {
        if (data.results.length !== 0) {
          data.results.forEach(function (track) {
            if (track.kind == 'song') {
              var releaseDate =
                typeof track.releaseDate !== 'undefined' &&
